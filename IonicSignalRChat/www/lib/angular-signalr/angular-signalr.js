  ...
  provider('hubFactory', function() {
    ...
    this.$get = ['$rootScope', '$timeout', '$', function($rootScope, $timeout, $) {
      ...
      return function hubFactory (hubName, opts) {
        ...
        if (opts.clientEvents.length > 0) {
            wrappedHub.forward(opts.clientEvents);
        }

        //auto-establish connection with server
        //wrappedHub.promise = wrappedHub.connect({ transport: ['webSockets',	'foreverFrame', 'serverSentEvents', 'longPolling'] });
